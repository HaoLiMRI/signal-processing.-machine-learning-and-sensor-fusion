...
"-------------------------------------------------------------------------------------------------"
"""
This is a demo code of Epsilon-ResNeXt.
    1) Epsilon-ResNeXt model is combination between ResNeXt and Epsilon-ResNet model proposed in [8] and [11]
    2) ResNeXt[8] in theory, works better than ResNet[1](which works as almost convex problem[2]), almost equals to performance of DenseNet[10] as comparaion of 6.figure of[9].
    2) Epsilon-ResNet[11] is proposed for reducing the size/number of parameter used in the network architecture. The "gate" residual block in Epsilon-ResNet could be implemented by
...
class Normal_Residual_Block(nn.Module): #----- In this version, the "cardinality" is implemented by using property "group" in PyTorch for each conv layer
    ...
    def forward(self, x):
        ...
        if self.gate_in_use == True:
            y, _ = tc.max(tc.nn.functional.relu(out - self.epsilon, inplace = True) + tc.nn.functional.relu(-out - self.epsilon, inplace = True))
...
class BottleNeck_Residual_Block(nn.Module): #----- In this version, the "cardinality" is implemented by using property "group" in PyTorch for each conv layer
    ...
    def __init__(self, number_in_channel, number_out_channel, stride = 1, linear_projection = None, num_of_group = 32, gate_in_use = True, EPSILON = 1): #----- num_of_group is "cardinality"
        ...
        self.normal_path = nn.Sequential(
                ...
                nn.ReLU(inplace = True), #-----inplace = True could overwrite the input of ReLU by using output to save memory(ReLU only needs output to calculate gradient )
                nn.Conv2d(int(number_out_channel/2), int(number_out_channel/2), 3, stride, 1, groups = num_of_group, bias = False),
                nn.BatchNorm2d(int(number_out_channel/2)),
