...
class STN3d(nn.Module):
    """
    T-Net which provides (3 * 3) transformation matrix
    """
    def __init__(self):
    ...
    def forward(self, x):
        ...
        if x.is_cuda:
            iden = iden.cuda()
        x = x + iden
        x = x.view(-1, 3, 3) # The output is a (3 x 3) tensor
        return x


class STNkd(nn.Module):
    """
    T-Net which provides (k * k) transformation matrix
    """
    ...
    def forward(self, x):
        ...
        if x.is_cuda:
            iden = iden.cuda()
        x = x + iden
        x = x.view(-1, self.k, self.k) # The output is (k by k) tensor
        return x


class PointNetEncoder(nn.Module):
    """
    Main, common framework of PointNet(common framework to exac local and global features)
    """
    def __init__(self, global_feat = True, feature_transform = False, semseg = False):
        super(PointNetEncoder, self).__init__()
        # Use 9x9 T-Net if the task is sementic segmentation, otherwise use 3x3 T-Net for other task
        self.stn = STN3d() if not semseg else STNkd(k=9)
        # Note here the size of input data to this conv layer is actual \
        # "1 x n x k", so we could use Conv1d with 1x1 kernel(conv filter) \
        # with k channels(so in total having 1 x 1 x k = k weights, here k = 3 or 9 depends on the task) to realize "64 x 64 mlp(multi-layer perceptron)" 
        self.conv1 = torch.nn.Conv1d(3, 64, 1) if not semseg else torch.nn.Conv1d(9, 64, 1)
        # Similarly, we can use Conv1d with 1x1 kernel(conv filter) with k channels(so in total having 1 x 1 x c = c weights, here c = 64) to realize "128 x 128 mlp(multi-layer perceptron)" 
        self.conv2 = torch.nn.Conv1d(64, 128, 1)
        # Similar way of using Conv1d to realize mlp
        self.conv3 = torch.nn.Conv1d(128, 1024, 1)
        # Batch norm over all out_channels
        self.bn1 = nn.BatchNorm1d(64)
        self.bn2 = nn.BatchNorm1d(128)
        self.bn3 = nn.BatchNorm1d(1024)
...
class PointNetCls(nn.Module):
    """
    Final PointNet stage for classification task
    """
...
class PointNetSeg(nn.Module):
    """
    Final PointNet stage for segmentation task
    """
